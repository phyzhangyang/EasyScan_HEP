...
class PROGRAM:
    ...
    def checkVar_replace(self, fileID, fileFlag, ParallelAdd):
            ## For 'Replace' method
            ...
            if file_flag:
                try :
                    ...
                    infile = open(ParallelAdd+self._InputFile[ii]).read()
                    ...
                    if os.path.exists(ParallelAdd+self._InputFile[ii]+'.ESbackup'):
                        try:
                            infile_bk = open(ParallelAdd+self._InputFile[ii]+'.ESbackup').read()
                            BackupFlag = True
                        except:
                            BackupFlag = False
                    else:
                ...
                ## Stop if easyscan could not read input file or input file.ESbackup.
                    ...
                    af.ErrorStop('Can not find/open the input file "%s" in program "%s".'%(ParallelAdd+self._InputFile[ii],self._ProgName))
            ...
            for jj in self._InRepVar[ii]:
                ...
                if file_flag:
                    ...
                    if len(match)==0:
                        if not BackupFlag:
                            af.ErrorStop( 'For input variable "%s" in program "%s" with "Replace" method, can not find "%s" in coressponding input file "%s" and its ESbackup file.'%(jj[0],self._ProgName,jj[3],ParallelAdd+self._InputFile[ii]) )
                        else:
                            ...
                            if len(bk_match)==0:
                                ...
                                BackupFlag = False
                                af.WarningNoWait( 'For input variable "%s" in program "%s" with "Replace" method, the input file "%s" does not contain "%s", but ESbackup file exists and contain it. In the following, ESbackup file will replace the content of the original input file.'%(jj[0],self._ProgName,ParallelAdd+self._InputFile[ii],jj[3]) )
                    if len(match)>1:
                        af.WarningNoWait( 'For input variable "%s" in program "%s" with "Replace" method, find %i "%s" in coressponding input file "%s". They will all be replaced by variable "%s" in the following program.'%(jj[0],self._ProgName,len(match),jj[3],ParallelAdd+self._InputFile[ii],jj[0]) )
            ...
            if file_flag:
                if len(self._InRepVar[ii])>0:
                    open(ParallelAdd+self._InputFile[ii]+'.ESbackup','w').write(infile)
                    open(ParallelAdd+self._InputFile[ii],'w').write(infile)


    def setInputVar(self, inputvar, ParallelThreads):
