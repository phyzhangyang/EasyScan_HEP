...
class program:
    ...
    def checkInOutVar_file(self, fileID):
            ## For 'File' method
    ...
    def checkInOutVar_position(self, fileID, fileFlag, status):
            ## For 'Position' method
            ...
            if status.lower()=="invar":
               ...
               self._InPosVar[ii] = [x for x in self._OutputVar if (x[1] == ii) and (x[2].lower() == 'position')]
            else:
               return
    ...
    def checkInOutVar_label(self, fileID, fileFlag):
            ## For 'Label' method
    ...
    def checkInOutVar_slha(self, fileID, fileFlag):
            ## For 'SLHA' method
    ...
    def checkInOutVar_replace(self, fileID, fileFlag):
            ## For 'Replace' method
    ...
    def setInputVar(self, inputvar):
        ...
        for ii in self._InFileID:
            ## For 'File' method 
            ## check the input vars that use 'File' method
            file_flag=self.checkInOutVar_file(ii)

            ## For 'Replace' method
            ## check the input vars that use 'Replace' method
            self.checkInOutVar_replace(ii, file_flag)
            
            ## For 'Position' method
            ## check the input vars that use 'Position' method
            self.checkInOutVar_position(ii, file_flag, "invar")
                         
            ## For 'Label' method
            ## check the input vars that use 'Label' method
            self.checkInOutVar_label(ii, file_flag)
                                
            ## For 'SLHA' method
            ## check the input vars that use 'SLHA' method
            self.checkInOutVar_slha(ii, file_flag)
    ...
    def ReadOutputFile(self,par,path):
        for ii in self._OutFileID:
            ...
            for jj in self._OutFileVar[ii]:
                ...
                ##new 20180419 liang
                par[jj[0]] = self._OutputFile[ii].split('/')[-1]
...
#                labelinum = [xxi for xx,xxi in enumerate(oulines) if jj[3] in xx]
                ...
                if len(labeline)>1:
                    sf.ErrorStop( 'For output variable "%s" in program "%s" with "Label" method, there is %s "%s" in output file "%s". Please choose other method.'%(jj[0],self._ProgName,len(labelinum),jj[3],self._OutputFile[ii]) )
    ...
    def setBound(self, boundvar):
        ...
        for ii in self._BoundVar:
            ...
            if len(ii) == 3:
                try:
                    float(ii[1])
                    sf.Info('  NameID= %s \tMinLimit= %s \tMaxLimit= %s'%(ii[0],ii[1],ii[2]))
                except: 
                    if ii[1] not in ["<=", ">=", ">", "<", "==", "!="]:
        ...
        for ii in self._BoundVar:
            if len(ii)== 3:
                ...
                try:
                    ...
                    phy = phy and par[ii[0]] >= float(ii[1]) and par[ii[0]] <= float(ii[2])
            else:
                if not (ii[3].startswith('/home') or ii[3].startswith('~'))     :
                    ii[3]=os.path.join(sf.CurrentPath, ii[3])
                boundfile = numpy.loadtxt(ii[3])
                ...
                if par[ii[0]] < numpy.amin(x) or par[ii[0]] > numpy.amax(x):
                    sf.ErrorStop('"%s" less(greater) than min(max) of the first column in limit file "%s" in "Bound" in program "%s".'%(ii[0], ii[3],self._ProgName))
