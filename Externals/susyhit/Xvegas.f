...
C      SUBROUTINE INTEG(FXN,IDIM,IPOINT,ITER,IPOINT1,ITER1,ACC,RES,REL)
      SUBROUTINE INTEG(FXN,IDIM,IPRINT,IVEGAS,RES,REL)
...
C--------- CHANGE BY J. ZUNFT, 05/06/92------------------
      IF (DABS(TSI).LT.1.D-70) TSI = DSIGN(1.D-70,TSI)
C--------------------------------------------------------
      WGT=TI2/TSI
      SI=SI+TI*WGT
      SI2=SI2+TI2
      ...
      SCALLS=SCALLS+CALLS
C--------- CHANGE BY J. ZUNFT, 05/06/92------------------
      IF (DABS(SWGT).LT.1.D-70) SWGT = DSIGN(1.D-70,SWGT)
      IF (DABS(SI2) .LT.1.D-70) SI2  = DSIGN(1.D-70,SI2)
C--------------------------------------------------------
      AVGI=SI/SWGT
      SD=SWGT*IT/SI2
      CHI2A=0.
...
C--------- CHANGE BY J. ZUNFT, 05/06/92------------------
21    IF (DABS(AVGI).LT.1.D-70) AVGI = DSIGN(1.D-70,AVGI)
C--------------------------------------------------------
...
C------------------------------------------------
      XO=DT(J)/D(I,J)
C--------- CHANGE BY J. ZUNFT, 05/06/92 ---------
      IF (XO.EQ.1.D0) XO = 0.99999999999999999D0
C------------------------------------------------
...
C SAVING THE RANDOM NUMBER GENERATOR CONFIGURATION
        ...
        REAL U(97)
        COMMON /SET1/ U,C,CD,CM,I,J
        OPEN(IUNIT,FILE='TESTRN',FORM='UNFORMATTED')
